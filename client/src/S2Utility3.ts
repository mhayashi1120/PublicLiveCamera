// TODO this module should be obsoleted.
...
const SYSTEM_LEVEL_NOTCH: number = 3;
...
export function getNodeSegments(lat1: number, lng1: number, lat2: number, lng2: number): NodeSegment[] {
  const key1 = S2.latLngToKey(lat1, lng1, 30)
  const key2 = S2.latLngToKey(lat2, lng2, 30)
  const key3 = S2.latLngToKey(lat1, lng2, 30)
  const key4 = S2.latLngToKey(lat2, lng1, 30)
  const keyCenter = S2.latLngToKey((lat1 + lat2) / 2, (lng1 + lng2) / 2, 30)
  const segs = [key1, key2, key3, key4, keyCenter].map(k => splitSegment(k));
  // Just check NorthEast and SouthWest segment
  const seg1 = splitSegment(key1);
  const seg2 = splitSegment(key2);

  let level = 1;
  let isSameCell = seg1[0] === seg2[0];

  for (let i = 1; i < seg1.length && i < seg2.length; i++) {
    for (let j = 0; j < SYSTEM_LEVEL_NOTCH && level <= S2.MAX_LEVEL; j++, level++) {
      const neighbors = S2.latLngToNeighborKeys(lat1, lng1, level);
      const k1 = S2.latLngToKey(lat1, lng1, level);
      const k2 = S2.latLngToKey(lat2, lng2, level);
      ...
      // split the cell by distance
      if (isSameCell || neighbors.includes(k2)) {
        const paths = segs.map(s => s.slice(0, i));

        const res = [];

        for (let k = 0; k < paths.length; k++) {
          const p = paths[k];
          if (!paths.slice(k + 1).find(p2 => segmentsEquals(p, p2))) {
            res.push(p);
          }
        }

        return res.map(s => NodeSegment.FromSegment(s));
      }
    }
  }
